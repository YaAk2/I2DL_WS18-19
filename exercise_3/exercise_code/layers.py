...
def conv_forward_naive(x, w, b, conv_param):
    ...
    for n in range(N):
        ...
        for c in range(C):
            x_padded.append(np.pad(x[n][c], pad, 'constant', constant_values=(0)))
            
        for h in range(0, H + 2*pad, 1):
            for i in range(0, W + 2*pad, stride):
                current_conv = 0
                for c in range(C):
                    for f in range(F):
                        current_conv = current_conv + np.sum(x_padded[c][h:h+HH, i:i+WW]*w[f][c])
                out.append(current_conv)
